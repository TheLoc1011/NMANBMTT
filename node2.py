...
HOST = 'localhost'
PORT = 9002
STORAGE_PATH = 'plan_node2.txt'
PRIVATE_KEY = None

# Tạo khóa riêng giả lập nếu cần
if not os.path.exists('private_key_node2.pem'):
    private_key, public_key = CryptoUtils.generate_rsa_key_pair()
    with open('private_key_node2.pem', 'wb') as f:
        f.write(private_key)
    with open('public_key_node2.pem', 'wb') as f:
        f.write(public_key)

with open('private_key_node2.pem', 'rb') as f:
    PRIVATE_KEY = f.read()
...
s.bind((HOST, PORT))
s.listen(5)
print(f"📡 Node 2 đang lắng nghe tại {HOST}:{PORT}...")
...
while True:
    ...
    try:
        ...
        # Kiểm tra chữ ký
        with open('public_key_node2.pem', 'rb') as f:
            pub = f.read()
        ...
        conn.sendall(b'ACK')
        print("✅ File đã được lưu tại node 2")
